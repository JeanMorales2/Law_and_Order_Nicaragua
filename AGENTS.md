...
- La app Expo usa `React Navigation` con una pila raíz mínima en `src/navigation/RootNavigator.tsx`.
- Por ahora solo existe la pantalla `StyleguideScreen`; los tabs por rol se conectarán después sobre esa navegación raíz.
- `React Query` se configura globalmente en `App.tsx` con `QueryClientProvider`.
- `Zustand` se usa para estado local de UI; el primer store es `src/store/uiStore.ts`.
...
- El cliente móvil todavía no conecta pantallas reales ni tabs por rol; hoy expone solo la pantalla visual `Styleguide`.
